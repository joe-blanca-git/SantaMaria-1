...
new_func = '''async def conciliar_composicao_ws(wb, acr_file, rows_to_export, font_header, font_body, align_center, align_left, align_right):
    ...
    # Colunas fixas conforme padrão do ACR nas Prorrogações
    col_titulo = 3 # Coluna D
    col_parcela = 4 # Coluna E
    col_saldo = -1
    
    # Buscar dinamicamente a coluna "saldo" ou "valor" no cabeçalho
    ...
    if col_saldo == -1:
        # Se não encontrou "saldo", vamos assumir uma coluna comum para valor como fallback
        col_saldo = 17 # Coluna R (exemplo de fallback)
    ...
    for idx, row in df_acr.iterrows():
        ...
        # Filtro da Parcela (sempre 01) igual nas Prorrogações
        parcela_val = str(row[col_parcela]).strip().split('.')[0]
        try:
            if int(parcela_val) != 1:
                continue
        except ValueError:
            # Se não conseguiu converter para int, pode ser cabeçalho ou lixo
            pass
        ...
        try:
            ...
            acr_data[titulo] = abs(saldo_float) # usar abs caso venha negativo no ACR
        except ValueError:
            continue
    ...
    for i, item in enumerate(rows_to_export):
        ...
        # O ACR pode não ter zeros a esquerda. Vamos verificar exact match ou lstrip match
        if nf_str in acr_data:
            found_key = nf_str
        elif nf_clean in acr_data:
            found_key = nf_clean
        else:
