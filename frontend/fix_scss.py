import os

path = r'c:\Users\tania.canedo\Documents\git\SantaMaria\frontend\src\styles\_global.scss'
with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

# The content got duplicated at line 36.
# Let's split it based on the first occurrence of `::-webkit-scrollbar-thumb {`
# and reconstruct the file cleanly.

expected_content = """@import 'styles/variables';

// Global utility classes that aren't in Bootstrap

.text-primary { color: $primary !important; }
.text-secondary { color: $gray-600 !important; }
.text-success { color: $success !important; }
.text-error { color: $danger !important; }
.text-warning { color: $warning !important; }

.bg-primary-light { background-color: rgba($primary, 0.1) !important; }
.bg-success-light { background-color: rgba($success, 0.1) !important; }
.bg-error-light { background-color: rgba($danger, 0.1) !important; }
.bg-warning-light { background-color: rgba($warning, 0.1) !important; }

html, body {
  margin: 0;
  padding: 0;
  font-family: $font-family-base;
  background-color: $body-bg;
  color: $body-color;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
}

// Custom Scrollbar minimalista
::-webkit-scrollbar {
  width: 6px;
  height: 6px;
}
::-webkit-scrollbar-track {
  background: transparent;
}
::-webkit-scrollbar-thumb {
  background-color: $gray-300;
  border-radius: $border-radius-lg;
}
::-webkit-scrollbar-thumb:hover {
  background-color: $gray-400;
}

// Flatpickr Customizations (Ant Design / Premium Look)
.flatpickr-calendar {
  font-family: $font-family-base !important;
  border-radius: 12px !important;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08) !important;
  border: 1px solid rgba(0,0,0,0.08) !important;
  padding: 8px 0;
  
  &.arrowTop:before, &.arrowTop:after {
    display: none !important;
  }
}

.flatpickr-day {
  border-radius: 4px !important;
  
  &.selected, &.startRange, &.endRange, &.selected.inRange, &.startRange.inRange, &.endRange.inRange, &.selected:focus, &.startRange:focus, &.endRange:focus, &.selected:hover, &.startRange:hover, &.endRange:hover, &.selected.prevMonthDay, &.startRange.prevMonthDay, &.endRange.prevMonthDay, &.selected.nextMonthDay, &.startRange.nextMonthDay, &.endRange.nextMonthDay {
    background: $primary !important;
    border-color: $primary !important;
    color: #fff !important;
  }
  
  &.inRange, &.prevMonthDay.inRange, &.nextMonthDay.inRange, &.today.inRange, &.prevMonthDay.today.inRange, &.nextMonthDay.today.inRange, &:hover, &.prevMonthDay:hover, &.nextMonthDay:hover, &:focus, &.prevMonthDay:focus, &.nextMonthDay:focus {
    background: rgba($primary, 0.1) !important;
    border-color: transparent !important;
  }
}

.flatpickr-months .flatpickr-month {
  height: 40px !important;
}

.flatpickr-current-month {
  font-size: 1rem !important;
  font-weight: 600 !important;
}

span.flatpickr-weekday {
  font-weight: 600 !important;
  color: $gray-500 !important;
}
"""

with open(path, 'w', encoding='utf-8') as f:
    f.write(expected_content)
