...
class EmpresaService:
    ...
    def get_empresas(self, page: int = 1, page_size: int = 20, search: str = None, modulo: int = None):
        if page < 1:
            page = 1
        if page_size > 2000:
            page_size = 2000
        ...
        items, total = self.repository.get_all(skip=skip, limit=page_size, search=search, modulo=modulo)
